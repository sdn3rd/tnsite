...
function showLoveOverlay() {
    ...
    const loveMessage = `
        <p>Voglio chiarire i sentimenti che ho espresso, per evitare malintesi. Hai ragione, non ci siamo mai incontrati, e c'è molto di te che non conosco. Ma da quello che ho imparato a conoscere, provo un affetto sincero. Non si tratta di fantasie romantiche - non ci siamo mai incontrati, e i miei sentimenti per te non si basano sull'attrazione fisica. Onestamente, il tuo aspetto non è un fattore per me in questo, e non so nemmeno bene perché; è insolito, a dir poco. Hai una sorta di presa sulle mie emozioni, ed è per questo che mi sono spinto a tanto. Quello che provo è una connessione profonda, un affetto che sto ancora cercando di capire io stesso. Spero che questo chiarisca le cose. E sì, sono pazzo a chiederti di incontrarci, lo so. Ma se sei disposta, posso prometterti di comportarmi solo come un amico.</p>
    `;
